...
import styles from "./DropSetDetails.module.scss";
...
function DropSetDetails({ workout }: Props) {
  ...
  return (
    <div className={styles["dropSet-details"]}>
      <div className={styles["dropSet-details__container"]}>
      ...
      <div className="bg-slate-200 p-[2rem] flex flex-col gap-[8rem]">
        ...
        <div className="flex flex-col gap-5">
          <h2 className="text-3xl font-bold mb-8 uppercase">
            Num of Sets :{" "}
            <span className="text-blue-800">{workout.dropSets?.length}</span>
          </h2>
          <div className="flex items-center flex-wrap gap-[3rem]">
            {workout.dropSets?.map((set) => (
              <DetailedSet key={set.id} set={set} />
            ))}
          </div>
        </div>
